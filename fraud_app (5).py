...
st.markdown("""
# 🕵️‍♂️ AI-Powered Job Fraud Detection
...
if uploaded_file is not None:
    try:
        ...
        if model is not None and tfidf is not None:
            ...
            user_threshold = st.slider("🎯 Select Fraud Probability Threshold", 0.0, 1.0, float(best_threshold), 0.01)
            test_df['fraud_predicted'] = (test_df['fraud_probability'] >= user_threshold).astype(int)
            ...
            if search:
                filtered_df = test_df[test_df['title'].str.contains(search, case=False)]
            else:
                filtered_df = test_df
            ...
            st.dataframe(
                filtered_df[['title', 'location', 'fraud_probability', 'fraud_predicted']]
                .sort_values(by='fraud_probability', ascending=False)
                .style.applymap(highlight_fraud, subset=['fraud_predicted'])
            )
            ...
            with col2:
                ...
                st.metric("Predicted Fraudulent Jobs", sizes[1])

            with st.expander("🔍 View Full Detailed Data"):
